...
st.subheader("Tambahan code")
st.subheader("Rata-rata Bonus per Department")

avg_bonus = df.groupby('Department')['Bonus'].mean()
st.bar_chart(avg_bonus)
st.subheader("Rata-rata Gaji Tahunan per Department")

avg_salary = df.groupby('Department')['AnnualSalary'].mean()
